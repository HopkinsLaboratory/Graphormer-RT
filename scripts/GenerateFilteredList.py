    ...
    for root, dirs, files in os.walk(path):
        for dir in dirs:
            ...
            if pB[0] < 50:
                ...
                A_ind = np.argmax(np.asarray(A_mobile[1:,:], dtype=float))
            ...
            try:
                ...
                if 'iproh' in A_solv.lower() or 'acetone' in A_solv.lower():
                    filter_list.append(dir)
                    # print(dir, "ACETONE")
                    continue
                if 'iproh' in B_solv.lower() or 'acetone' in B_solv.lower():
                    filter_list.append(dir)
                    # print(dir, "ACETONE")
                    continue
                # print(A_solv, B_solv, dir)
            except:
                # print('no gradient')
                filter_list.append(dir)
                continue
        ...
        with open(r'/home/cmkstien/Desktop/RT_data/filtered_July31/filtered_list_july31.csv', 'w') as file:
            for item in list(set(filter_list)):
                # print(item)
                file.write("%s\n" % item)
